...
export const useCountriesAirportsRoute = (): UseCountriesAirortsRoute => {
    const match = useRouteMatch<CountriesAirportsRouteParams>(ROUTES.API_TESTING_COUNTRIES_AIRPORTS);
    ...
    const setCountriesAirportsRoute = useCallback(
        (countryId: string) => {
            const path = generatePath(ROUTES.API_TESTING_COUNTRIES_AIRPORTS, {id: countryId});
            history.push(path);
        },
        [history],
    );
